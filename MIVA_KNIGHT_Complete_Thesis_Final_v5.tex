...
% RL injection v2 (needed before global TikZ styles)
...
\tikzset{
  ...
  fusnode/.style={basebox,fill=fusioncolor!25,draw=fusioncolor,minimum width=3.2cm,minimum height=1.0cm,thick}
}
...
\State $\Pi_1 \gets$ \texttt{Your review has been verified (confidence: \{100*Omega:.0f\}\%).}
...
\For{$i = 1$ to $M$}
  ...
  \State $\Pi_i \gets$ \texttt{\{badge\} \{claim\_text\} | Sources: \{top\_ev\}}
\EndFor
...
\For{each $c_i \in C$}
  ...
  \State log: \texttt{Claim \{i\}: \{k\_i\} rounds, kappa=\{kappa\_i:.3f\}}
\EndFor
