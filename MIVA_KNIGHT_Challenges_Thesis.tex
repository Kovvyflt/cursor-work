...
With $K$ modality tokens and $h$ attention heads, the cross-modal Transformer can represent any pairwise cross-modal interaction. Specifically, each head $i$ can specialise in the interaction $(\eA, \eB_i)$ for $\binom{K}{2}$ distinct pairs, while the CLS-token mean-pooling aggregates all interactions linearly.
\end{theorem}
...
\begin{tikzpicture}[node distance=0.6cm and 1.1cm]
  ...
  \draw[redarr,dashed] (coloss.west) -- ++(-0.4,0) |- (weakNN.east)
      node[near end,right,font=\tiny]{update $\phi$\\(5.25M params)};
  \draw[arr,color=modalA,very thick,dashed] (strongNN.south) -- ++(0,-0.4) -- ++(0,-0.4)
      node[below,font=\small\bfseries,color=modalA]{knowledge\\transfer $\downarrow$} -- (weakNN.north);

  \node[font=\small\bfseries,color=navyblue] at ($(strongNN)!0.5!(weakNN)+(1.8,0)$) {2.23\% of\\235M params};
\end{tikzpicture}
\end{adjustbox}
    ...
    {\textbf{Aligned Output}\\[4pt]
     \begin{tikzpicture}[scale=0.6]
       \draw[modalA,thick,smooth,domain=0:4.5,samples=60]
         plot (\x, {0.35*sin(180*\x r)+0.15*sin(360*\x r)});
